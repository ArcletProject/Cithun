...
class AsyncPermissionService(Generic[T]):
    ...
    async def permission_on(
        ...
        async def _format_node(node: ResourceNode, prefix: str, is_last: bool):
            ...
            acl = await self.storage.get_primary_acl(subject, node.id)
