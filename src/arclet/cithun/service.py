...
class PermissionService(Generic[T]):
    ...
    def _check_acl_dependencies(
        ...
        current_subject = (
            self.storage.get_user(acl.subject_id)
            if acl.subject_type is SubjectType.USER
            else self.storage.get_role(acl.subject_id)
        )  # noqa: E501
        ...
        for dep in available_dependencies:
            if not dep.check_target_subject(context, current_subject):
                continue
        ...
        def _format_node(node: ResourceNode, prefix: str, is_last: bool):
            ...
            if show_dependencies and acl:
                deps = [
                    ...
                    if dep.target_resource_id == node.id and dep.check_target_subject(context, subject)
                ]
