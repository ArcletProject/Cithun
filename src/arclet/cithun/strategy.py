...
class AclDependency(Generic[T]):
    ...
    target_subject: User | Role | Callable[[T | None, User | Role], bool] | None = None
    depend_subject: User | Role | Callable[[T | None, User | Role], User | Role] | None = None
    required_mask: Permission = Permission.AVAILABLE

    def check_target_subject(self, context: T | None, current_subject: User | Role) -> bool:
        if self.target_subject is None:
            return True
        elif callable(self.target_subject):
            return self.target_subject(context, current_subject)
        else:
...
class PermissionEngine(Generic[T]):
    ...
    def depend(
        self,
        target_subject: User | Role | Callable[[T | None, User | Role], bool],
        target_resource_id: str,
        depend_resource_id: str,
        /,
        *,
        ...
        Args:
            target_subject (User | Role | Callable[[T | None, User | Role], bool]): 目标主体或目标检查函数。
            target_resource_id (str): 目标资源 ID。
            depend_resource_id (str): 依赖资源 ID。
            required_mask (Permission): 依赖所需的权限掩码。
        """
